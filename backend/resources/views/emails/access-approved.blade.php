...
<head>
    ...
    <style>
        ...
        .paket-info { background: #f0fdf4; border: 1px solid #bbf7d0; border-radius: 12px; padding: 16px; margin: 16px 0; }
        .paket-info .name { color: #166534; font-weight: 700; font-size: 16px; margin: 0 0 4px; }
        .paket-info .detail { color: #4ade80; font-size: 13px; margin: 0; }
...
<body>
    <div class="container">
        ...
        <div class="body">
            ...
            <p>Kabar baik! Admin telah menyetujui akses Anda ke paket tryout berikut:</p>

            <div class="success-box">
                <div class="icon">🎓</div>
                <h3>Akses Paket Aktif</h3>
                <p>Anda sekarang bisa mengerjakan tryout!</p>
            </div>

            <div class="paket-info">
                <p class="name">📦 {{ $paket->judul }}</p>
                <p class="detail">{{ $paket->jumlah_soal }} soal · {{ $paket->durasi }} menit · Passing grade {{ $paket->passing_grade }}</p>
            </div>

            <p>Silakan login ke akun TryoutPro Anda untuk mulai mengerjakan tryout.</p>

            <a href="{{ config('app.frontend_url', 'http://localhost:3000') }}/dashboard" class="cta">
                🚀 Mulai Tryout Sekarang
            </a>

            <p style="font-size:12px;color:#94a3b8;">Jika tombol di atas tidak berfungsi, salin link berikut: {{ config('app.frontend_url', 'http://localhost:3000') }}/dashboard</p>
        </div>
        <div class="footer">
